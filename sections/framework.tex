...
The first sequential Functor queries information from the Film Dactor instance which is needed to add the film to the Actor Dactor's filmography \gls{relation}, i.e. the films title.
...
The calling actor can then react to the failure by resume or restart the Functor, stop this computation, or escalate the failure to a up-stream actor.
...
The framework maintains a unified namespace in which each \gls{dactor} instance is identified by it's type and a unique id.
In fact, querying a specific \gls{dactor} requires just obtaining the messaging address from the nameservice and sending a message to it.
% TODO one line of code for this?
...
Building on the Akka frameworks parent-child supervision concept, our framework allows for transparent failure handling configurations.
